...
import { getReportedBugs } from '@/actions/manage/site';
...
function ReportedBugsPageComponent({ after }: { after?: string }) {
    ...
    const handleNextPage = () => {
        if (bugs.length > 0) {
            ...
            setPage(p => p + 1);
            router.push(`/manage/site/bugs?after=${lastId}`);
        }
    };
    ...
    const handlePrevPage = () => {
        ...
        setPage(p => p - 1);
        const url = prevAfterId ? `/manage/site/bugs?after=${prevAfterId}` : '/manage/site/bugs';
        router.push(url);
    };
                        ...
                        <TableBody>
                            {contentLoading ? (
                                ...
                                bugs.map((bug) => (
                                    <TableRow key={bug.id}>
                                        <TableCell>
                                            <Link href={`/manage/site/bugs/${bug.id}`} className="font-medium hover:underline">{bug.title}</Link>
                                        </TableCell>
