...
import { getSystemErrors } from "@/actions/manage/site";
...
function SystemErrorsPageComponent({ after }: { after?: string }) {
    ...
    const handleNextPage = () => {
        if (errors.length > 0) {
            ...
            setPage(p => p + 1);
            router.push(`/manage/site/errors?after=${lastId}`);
        }
    };
    ...
    const handlePrevPage = () => {
        ...
        setPage(p => p - 1);
        const url = prevAfterId ? `/manage/site/errors?after=${prevAfterId}` : '/manage/site/errors';
        router.push(url);
    };
                                ...
                                errors.map((error) => (
                                    <TableRow key={error.id}>
                                        ...
                                        <TableCell className="max-w-xs">
                                             <Link href={`/manage/site/errors/${error.id}`} className="hover:underline text-sm block truncate" target="_blank" rel="noopener noreferrer">
                                                {error.message}
                                            </Link>
                                        </TableCell>
                                    </TableRow>
