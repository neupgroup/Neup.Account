 ...
 *
 * The repository names these tables as AssetGroupInfo, AssetGroupMember,
 * AssetMemberRole, and Asset. These are the assetsProfile/assetsMember/
 * assetsRole/assetsElements tables referenced in the request.
 */
...
export async function checkPermission(input: CheckPermissionInput): Promise<PermissionCheckResult> {
	...
	try {
		...
		if (externalSession) {
			if (externalSession.appId !== app || externalSession.sessionKey !== skey) {
				return { status: 'invalid', reason: 'invalidSource' };
			}

			if (externalSession.expiresOn <= now) {
			...
			if (internalSession.authSessionKey !== skey) {
				return { status: 'invalid', reason: 'invalidSource' };
			}
